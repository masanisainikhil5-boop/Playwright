...
```
Error: locator.click: Error: strict mode violation: getByText('TypeScript') resolved to 2 elements:
    1) <span>TypeScript</span> aka getByRole('button', { name: 'TypeScript' })
    2) <div role="option" class="select-option" data-value="TypeScript">…</div> aka getByRole('option', { name: 'TypeScript' })

Call log:
  - waiting for getByText('TypeScript')
...
> 8  |     await page.getByText("TypeScript").click()
     |                                        ^ Error: locator.click: Error: strict mode violation: getByText('TypeScript') resolved to 2 elements:
  9  | 
  10 |     await page.locator("//div[@data-testid=('dropdown-experience')]").click()
  11 |     await page.getByText("Mid-level (4-6 years)").click()
  12 | 
  13 | 
  14 | })
```